module.exports = (app, connection) => {
    ...
    app.get('/creations/inprogress', (req, res) => {
		connection.query('SELECT id, titre, description FROM creation WHERE nomfichier IS NULL ORDER BY id DESC', (err, rows) => {
			if (err)
				res.send(err)
			res.send(rows)
		})
    ...
    app.get('/avencement',(req, res)=>{
		connection.query('SELECT creation.id, creation.titre, creation.description, etat_avancement.libelle,etat_avancement.valeuravancement, creation.miseajour FROM creation,etat_avancement WHERE etat_avancement.idcreation=creation.id AND creation.nomfichier is null', (err,rows)=>{
			if(err)
				res.send(err)
